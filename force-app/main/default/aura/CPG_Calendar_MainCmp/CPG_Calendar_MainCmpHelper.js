({
    ...
    createCalendarSummaryData : function(component,helper, summaryData){
        ...
        for(var monthIndex= 0 ; monthIndex< monthList.length; monthIndex++){
            ...
            for(var programTypeIndex= 0 ; programTypeIndex< programTypesInOrder.length; programTypeIndex++){
    ...
    trackDownloads : function(component,helper){
        var action = component.get("c.cacheDownloadInformation");
        action.setParams({
            "SearchTxt": "DownloadPDF",
        });
        action.setCallback(this, function (response) {
            var state = response.getState();
            if (state === "SUCCESS") {
